...
async function main() {
  ...
  const lockedPositions = await host.evaluate(`(() => {
    mathChallengeSets = [{ triggerSecond: 0, status: "pending", questions: [
      { expression: "7 + 5", answer: 12 }, { expression: "18 ÷ 3", answer: 6 }
    ] }];
  ...
  await host.evaluate(`mathChallengeAnswer.value = "12"; mathChallengeForm.querySelector('[type=submit]').click(); mathChallengeAnswer.value = "6"; mathChallengeForm.querySelector('[type=submit]').click()`);
  ...
  }, "phone controls return after the host answers both questions");
